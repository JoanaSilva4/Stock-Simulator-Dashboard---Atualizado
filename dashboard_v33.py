...
@st.cache_data(ttl=3600, max_entries=30)   # LRU: máx 30 ficheiros em RAM (~180 MB)
...
if "date" in df_filtered.columns and "soh_final" in df_filtered.columns:
    if mode == "By SKU":
        ...
        if df_asis is not None:
                df_asis_sku = df_asis[df_asis["sku"] == sku_sel].sort_values("date")
        ...
        if df_asis is not None:
            df_asis_cls = df_asis[df_asis["sku"].isin(skus_in_class)].copy()
...
if cs_c and ct_c and "date" in df_filtered.columns:
    ...
    df_mes["month_label"] = df_mes["date"].dt.strftime("%b %Y")   
    df_mes["month_sort"]  = df_mes["date"].dt.to_period("M").astype(str)  
