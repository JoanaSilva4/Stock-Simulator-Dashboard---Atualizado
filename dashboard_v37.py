...
if "date" in df_filtered.columns and "soh_final" in df_filtered.columns:
    if mode == "By SKU":
        ...
        if "stockout" in df_plot.columns:
            ...
            if not so.empty:
                df_plot["stockout_display"] = df_plot["stockout"].apply(lambda x: x if x > 0 else 0)
                
                # Trace real (pontos nos dias certos, invisíveis nos outros)
                fig.add_trace(go.Scatter(
                    x=df_plot["date"],
                    y=df_plot["soh_final"],
                    mode="markers",
                    ...
                    marker=dict(
                        ...
                        size=df_plot["stockout_display"].apply(lambda x: 8 if pd.notna(x) else 0),
                        symbol="cross",
                        line=dict(color="#C0392B", width=2)
                    ),
                    customdata=df_plot["stockout_display"],
