...
@st.cache_data(ttl=3600)          # ficheiros não mudam → 1 hora é seguro
...
def find_best(parquets_meta_frozen, abc_frozen, mode, sku_or_class, fill_rate_target=95.0):
    ...
    """
    # Reconstruir parquets_meta a partir do tuplo
    parquets_meta = [
        {"filepath": f, "policy": p, "ns": n, "k": k}
        for f, p, n, k in parquets_meta_frozen
    ]
    # Reconstruir df_abc a partir do tuplo (lista de (sku, classe))
    if abc_frozen:
...
# Garantir que df_asis está sempre definido mesmo que o sidebar não chegue a correr
df_asis = None
...
with st.sidebar:
    ...
    # Carrega dados AS IS se o ficheiro existir na mesma pasta
...
if mode == "Overview":
    ...
    # ── Compute overview KPIs (based on best combo per class if available, else global)
    if _overview_comparison_rows:
        avg_fr  = np.nanmean([r["TO BE SL (%)"]        for r in _overview_comparison_rows])
        avg_csl = np.nanmean([r["TO BE CSL"]            for r in _overview_comparison_rows])
        avg_cob = np.nanmean([r["TO BE Avg. Coverage"]  for r in _overview_comparison_rows])
        avg_ss  = np.nanmean([r["TO BE SS"]             for r in _overview_comparison_rows])
        avg_ct  = np.nanmean([r["TO BE Cost"]           for r in _overview_comparison_rows])
    else:
    ...
    # Nota: df_asis é opcional — SL e Coverage vêm de ASIS_CLASS_STATS (hardcoded).
    # O bloco aparece sempre que df_abc estiver disponível.
    if df_abc is not None:
