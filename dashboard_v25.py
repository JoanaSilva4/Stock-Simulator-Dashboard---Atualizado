...
with st.sidebar:
    ...
    if os.path.exists(_asis_path):
        try:
            ...
            st.caption(f"✓ AS IS stock file loaded")
        except Exception as _e:
            df_asis = None
            st.caption(f"⚠ AS IS stock file error: {_e}")
    else:
        df_asis = None
    ...
    if df_abc is not None:
        ...
        for cls in classes_list_asis:
            ...
            comparison_rows.append({
                ...
                "TO BE Avg. Coverage":round(tobe_cov, 1)  if not np.isnan(tobe_cov)   else np.nan,
