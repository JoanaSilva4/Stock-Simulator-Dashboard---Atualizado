...
ASIS_CLASS_STATS = {
    # classe: (SL %, avg_coverage_days)
    "AX": (98.54, 14), "AY": (89.47, 21), "AZ": (89.54, 13),
    "BX": (94.08, 17), "BY": (86.88, 19), "BZ": (78.27, 51),
    "CX": (95.33, 21), "CY": (84.47, 39), "CZ": (71.55, 67),
}
...
@st.cache_data(ttl=3600)
def _get_all_skus(first_filepath):
    """Lê apenas o primeiro ficheiro para extrair a lista de SKUs.
    Todos os ficheiros parquet têm os mesmos SKUs, portanto não é necessário
    iterar pelos 117 ficheiros — basta um."""
    try:
        import pyarrow.parquet as pq
        schema_cols = pq.read_schema(first_filepath).names
        if "sku" not in schema_cols:
            return []
        df_tmp = pd.read_parquet(first_filepath, columns=["sku"])
        return sorted(df_tmp["sku"].unique())
    except Exception:
        return []
...
@st.cache_data(ttl=3600, max_entries=8)    # LRU: máx 8 ficheiros em RAM (~50 MB)
...
def compute_metrics(df, policy=""):
    ...
    _pol = policy.lower().replace("-", "_")
    if _pol == "min_max":
        ss_c = "SS_s" if "SS_s" in df.columns else None
        m["ss_medio"]   = df[ss_c].mean() if ss_c else np.nan
        m["ss_medio_S"] = df["SS_S"].mean() if "SS_S" in df.columns else np.nan
    else:
        ss_c = next((c for c in ["SS","ss","safety_stock","SS_s"] if c in df.columns), None)
        m["ss_medio"]   = df[ss_c].mean() if ss_c else np.nan
        m["ss_medio_S"] = np.nan
...
def _compute_global_combos(parquets_meta_frozen):
    """Calcula fill_rate e cost_total globais (todos os SKUs juntos) para cada ficheiro parquet.
    Resultado cacheado — não recalcula a cada render do dashboard."""
    import pyarrow.parquet as pq
    rows = []
    for filepath, policy, ns, k in parquets_meta_frozen:
        try:
            schema_cols = pq.read_schema(filepath).names
            cols_to_load = [c for c in COLS_NEEDED if c in schema_cols]
            df_g = pd.read_parquet(filepath, columns=cols_to_load if cols_to_load else None)
            for col in df_g.select_dtypes(include=["float64"]).columns:
                df_g[col] = df_g[col].astype("float32")
            for col in df_g.select_dtypes(include=["int64"]).columns:
                df_g[col] = df_g[col].astype("int32")
            ...
            if np.isnan(fr_g) or np.isnan(ct_g) or ct_g == 0:
                continue
            rows.append({"policy": policy, "ns": ns, "k": k,
                         "fill_rate": fr_g, "cost_total": ct_g})
        except Exception:
            continue
    return rows
...
@st.cache_data(ttl=3600)
def load_asis(folder):
    """Carrega o ficheiro AS IS stock com cache para não re-ler a cada render."""
    _asis_path = os.path.join(folder, "20260210_stock.parquet")
    if not os.path.exists(_asis_path):
        return None
    try:
        return pd.read_parquet(_asis_path, columns=["sku", "date", "stock_on_hand_int"])
    except Exception:
        return None
...
def find_best(parquets_meta_frozen, abc_frozen, mode, sku_or_class, fill_rate_target=95.0):
    """
    ...
    Nota: recebe tuplos serializáveis para ser compatível com @st.cache_data.
    Converter internamente para lista de dicts.
    """
    import pyarrow.parquet as pq
    parquets_meta = [
        {"filepath": f, "policy": p, "ns": n, "k": k}
        for f, p, n, k in parquets_meta_frozen
    ]
    if abc_frozen:
        df_abc = pd.DataFrame(abc_frozen, columns=["sku", "Classe_ABC_XYZ"])
    else:
        df_abc = None
    rows = []
    for meta in parquets_meta:
        try:
            schema_cols = pq.read_schema(meta["filepath"]).names
            cols_to_load = [c for c in COLS_NEEDED if c in schema_cols]
            df = pd.read_parquet(meta["filepath"], columns=cols_to_load if cols_to_load else None)
            for col in df.select_dtypes(include=["float64"]).columns:
                df[col] = df[col].astype("float32")
            for col in df.select_dtypes(include=["int64"]).columns:
                df[col] = df[col].astype("int32")
...
with st.sidebar:
    ...
    if mode == "By SKU":
        ...
        all_skus = _get_all_skus(parquets_meta[0]["filepath"])
        if not all_skus:
    ...
    k_sel = None
    pol_disp = "" 
    fill_rate_target = 95.0  # fixed target for best combination finder
    ...
    if mode != "Overview":
        ...
        st.markdown(
            f"<div class='param-desc'>📋 <b>Policy</b> — Inventory management policy used in the simulation.</div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            f"<div class='param-desc'>⚖️ <b>k value</b> — Multiplication factor applied to the review period to obtain N.</div>",
            unsafe_allow_html=True
        )
        k_opts = sorted(set(
...
def load_all_metrics(parquets_meta_frozen):
    ...
    for (filepath, policy, ns, k) in parquets_meta_frozen:
        try:
            # Lê directamente sem passar pelo cache do load_parquet
            # para não acumular 135 ficheiros em memória
            schema_cols = pq.read_schema(filepath).names
            cols_to_load = [c for c in COLS_NEEDED if c in schema_cols]
            df = pd.read_parquet(filepath, columns=cols_to_load if cols_to_load else None)
            if "date" in df.columns:
                df["date"] = pd.to_datetime(df["date"], dayfirst=True, errors="coerce").dt.normalize()
            # Converter para tipos mais leves
            for col in df.select_dtypes(include=["float64"]).columns:
                df[col] = df[col].astype("float32")
            for col in df.select_dtypes(include=["int64"]).columns:
                df[col] = df[col].astype("int32")

            n_days_file = df["date"].nunique() if "date" in df.columns else len(df)

            for sku, grp in (df.groupby("sku") if "sku" in df.columns else [("all", df)]):
                m = compute_metrics(grp, policy=policy)
                m["sku"]    = sku
                m["policy"] = policy
                m["ns"]     = ns
                m["k"]      = k
            ...
            del df  # liberta memória imediatamente após processar
            gc.collect()
        except Exception:
            pass
...
if mode == "Overview":
    ...
    # ── Pre-compute comparison_rows (best combo per class) for use in overview KPIs
    _overview_comparison_rows = []
    if df_abc is not None:
        _asis_sku_avg_ov = None
        if df_asis is not None:
            _asis_sku_avg_ov = (
                df_asis.groupby("sku")["stock_on_hand_int"].mean()
                .reset_index()
                .rename(columns={"stock_on_hand_int": "asis_stock_medio"})
                .merge(df_abc, on="sku", how="left")
            )
        _pm_frozen_ov  = tuple((m["filepath"], m["policy"], m["ns"], m["k"]) for m in parquets_meta)
        _abc_frozen_ov = tuple(df_abc[["sku","Classe_ABC_XYZ"]].itertuples(index=False, name=None))
        for _cls_ov in sorted(df_abc["Classe_ABC_XYZ"].dropna().unique()):
            _skus_ov = df_abc[df_abc["Classe_ABC_XYZ"] == _cls_ov]["sku"].unique()
            _res_ov  = find_best(_pm_frozen_ov, _abc_frozen_ov, "CLASS", _cls_ov, fill_rate_target=fill_rate_target)
            if _res_ov:
                _best_ov, _, _ = _res_ov
                _meta_ov = next(
                    (m for m in parquets_meta
                     if m["policy"] == _best_ov["policy"]
                     and m["ns"] == _best_ov["ns"]
                     and m["k"] == _best_ov["k"]),
                    None
                )
                if _meta_ov:
                    _df_ov = load_parquet(_meta_ov["filepath"])
                    _df_cls_ov = _df_ov[_df_ov["sku"].isin(_skus_ov)]
                    _m_ov = compute_metrics(_df_cls_ov)
                    _n_skus_ov = len(_skus_ov)
                    _overview_comparison_rows.append({
                        "TO BE SL (%)":        _m_ov.get("fill_rate", np.nan),
                        "TO BE CSL":           _m_ov.get("csl", np.nan),
                        "TO BE Avg. Coverage": _m_ov.get("cobertura", np.nan),
                        "TO BE Stock":         _m_ov.get("stock_medio", np.nan),
                        "TO BE Cost": _m_ov.get("cost_total", np.nan) / _n_skus_ov if _n_skus_ov > 0 else np.nan,
                    })

    # ── Compute overview KPIs 
    if _overview_comparison_rows:
        n_skus_por_classe = {
            cls: len(df_abc[df_abc["Classe_ABC_XYZ"] == cls])
            for cls in df_abc["Classe_ABC_XYZ"].dropna().unique()
        }
        classes_com_dados = [
            cls for cls in df_abc["Classe_ABC_XYZ"].dropna().unique()
            if cls in [r.get("_classe") for r in _overview_comparison_rows]
        ]
    
        for r, cls in zip(_overview_comparison_rows, sorted(df_abc["Classe_ABC_XYZ"].dropna().unique())):
            r["_n_skus"] = n_skus_por_classe.get(cls, 1)
    
        total_skus = sum(r["_n_skus"] for r in _overview_comparison_rows)
    
        def wavg(key):
            vals = [(r[key], r["_n_skus"]) for r in _overview_comparison_rows if not np.isnan(r.get(key, float("nan")))]
            if not vals: return np.nan
            return sum(v * w for v, w in vals) / sum(w for _, w in vals)
    
        avg_fr  = wavg("TO BE SL (%)")
        avg_csl = wavg("TO BE CSL")
        avg_cob = wavg("TO BE Avg. Coverage")
        avg_stk = wavg("TO BE Stock")
        avg_ct  = wavg("TO BE Cost")
    else:
        ...
        avg_stk = df_all["stock_medio"].mean() if "stock_medio" in df_all else np.nan
    ...
    st.markdown("<div class='sec-hdr'>◆ Simulated Performance (avg across all SKUs & best combinations for each class)</div>",
                unsafe_allow_html=True)
    ...
    for col, val, lbl, cls in [
        ...
        (c3, f"{avg_cob:,.0f} days" if not np.isnan(avg_cob) else "—", "Avg Stock Coverage",""),
        (c4, f"{avg_stk:,.1f} units" if not np.isnan(avg_stk) else "—", "Avg Stock",         ""),
        (c5, f"€{avg_ct:,.0f}" if not np.isnan(avg_ct)  else "—", "Avg Total Cost / SKU",  "orange"),
    ]:
        with col:
            st.markdown(f"""
            <div class="ovcard {cls}">
    ...
    if df_abc is not None and "sku" in df_all.columns:
        ...
        best_rows = []
        _pm_frozen  = tuple((m["filepath"], m["policy"], m["ns"], m["k"]) for m in parquets_meta)
        _abc_frozen = tuple(df_abc[["sku","Classe_ABC_XYZ"]].itertuples(index=False, name=None)) if df_abc is not None else ()
        for cls in classes_list:
            skus_cls = df_abc[df_abc["Classe_ABC_XYZ"] == cls]["sku"].unique()
            res_cls = find_best(_pm_frozen, _abc_frozen, "CLASS", cls, fill_rate_target=fill_rate_target)
            if res_cls:
                best_cls, _, _ = res_cls
                best_rows.append({
                    "Class": cls,
                    "Best Combination": f"{policy_label(best_cls['policy'])} · SL {best_cls['ns']}% · k={best_cls['k']}",
    ...
    if df_abc is not None:
        st.markdown("<div class='sec-hdr'>◆ AS IS vs TO BE — Comparison by Class</div>",
                    unsafe_allow_html=True)

        # Stock médio AS IS: só calculado se df_asis estiver disponível
        if df_asis is not None:
            asis_sku_avg = (
                df_asis.groupby("sku")["stock_on_hand_int"].mean()
                .reset_index()
        ...
        classes_list_asis = sorted(df_abc["Classe_ABC_XYZ"].dropna().unique())
        comparison_rows = []

        _pm_frozen2  = tuple((m["filepath"], m["policy"], m["ns"], m["k"]) for m in parquets_meta)
        _abc_frozen2 = tuple(df_abc[["sku","Classe_ABC_XYZ"]].itertuples(index=False, name=None)) if df_abc is not None else ()
        for cls in classes_list_asis:
            skus_cls = df_abc[df_abc["Classe_ABC_XYZ"] == cls]["sku"].unique()
            asis_sl, asis_cov = ASIS_CLASS_STATS.get(cls, (np.nan, np.nan))
            if asis_sku_avg is not None:
                asis_stock = asis_sku_avg[asis_sku_avg["Classe_ABC_XYZ"] == cls]["asis_stock_medio"].mean()
            else:
                asis_stock = np.nan

            res_cls = find_best(_pm_frozen2, _abc_frozen2, "CLASS", cls, fill_rate_target=fill_rate_target)
            if res_cls:
                best_cls, _, _ = res_cls
                best_meta = next(
                    (m for m in parquets_meta
                     if m["policy"] == best_cls["policy"]
                     and m["ns"] == best_cls["ns"]
                     and m["k"] == best_cls["k"]),
                    None
                )
                if best_meta:
                    df_best_f = load_parquet(best_meta["filepath"])
                    df_best_cls_rows = df_best_f[df_best_f["sku"].isin(skus_cls)]
                    m_tobe = compute_metrics(df_best_cls_rows)
                    tobe_sl    = m_tobe.get("fill_rate", np.nan)
                    tobe_cov   = m_tobe.get("cobertura", np.nan)
                    tobe_stock = df_best_cls_rows["soh_final"].mean() if "soh_final" in df_best_cls_rows.columns else np.nan
                    best_combo_lbl = f"{policy_label(best_cls['policy'])} · NS {best_cls['ns']}% · k={best_cls['k']}"
                else:
                    tobe_stock = tobe_sl = tobe_cov = np.nan
                    best_combo_lbl = "—"
            else:
                tobe_stock = tobe_sl = tobe_cov = np.nan
                best_combo_lbl = "—"
            ...
            comparison_rows.append({
                "Class":             cls,
                "AS IS SL (%)":       asis_sl,
                "AS IS Avg. Coverage":asis_cov,
                "AS IS Avg. Stock":   round(asis_stock, 1) if not np.isnan(asis_stock) else np.nan,
                "TO BE SL (%)":       round(tobe_sl, 2)   if not np.isnan(tobe_sl)    else np.nan,
                "TO BE Avg. Coverage":round(tobe_cov, 0)  if not np.isnan(tobe_cov)   else np.nan,
                "TO BE Avg. Stock":   round(tobe_stock, 1) if not np.isnan(tobe_stock) else np.nan,
            })

        df_comparison = pd.DataFrame(comparison_rows)
        fmt_comp = {
            "AS IS SL (%)":        "{:.2f}%",
            "AS IS Avg. Coverage": "{:.0f} d",
            "AS IS Avg. Stock":    "{:,.1f}",
            "TO BE SL (%)":        "{:.2f}%",
            "TO BE Avg. Coverage": "{:.0f} d",
            "TO BE Avg. Stock":    "{:,.1f}",
        }
        styled_comp = df_comparison.style.format(fmt_comp, na_rep="—") \
            .background_gradient(
                subset=["AS IS SL (%)", "TO BE SL (%)"],
    ...
    with st.spinner("Evaluating all combinations globally..."):
        global_rows = _compute_global_combos(
            tuple((m["filepath"], m["policy"], m["ns"], m["k"]) for m in parquets_meta)
        )
...
if "date" in df_filtered.columns and "soh_final" in df_filtered.columns:
    if mode == "By SKU":
        ...
        if "stockout" in df_plot.columns:
            ...
            if not so.empty:
                df_plot["stockout_display"] = df_plot["stockout"].apply(lambda x: x if x > 0 else None)
                df_plot["stockout_hover"] = df_plot["stockout"].apply(lambda x: x if x > 0 else 0)  # ← nova coluna para hover
                ...
                fig.add_trace(go.Scatter(
                    ...
                    showlegend=False,
                    marker=dict(
                        color=df_plot["stockout_display"].apply(
                            lambda x: "#C0392B" if pd.notna(x) else "rgba(0,0,0,0)"
                        ),
                        size=df_plot["stockout_display"].apply(lambda x: 9 if pd.notna(x) else 0),
                        symbol="cross",
                        line=dict(color="#C0392B", width=2)
                    ),
                    customdata=df_plot["stockout_hover"],  # ← usa a coluna com 0 em vez de None
                    hovertemplate="Stockout: %{customdata:.0f}<extra></extra>",
                ))
        
                # Trace fantasma só para a legenda
                fig.add_trace(go.Scatter(
                    x=[None], y=[None],
                    mode="markers",
                    name="Stockout",
        ...
        if df_asis is not None:
                df_asis_sku = df_asis[(df_asis["sku"] == sku_sel) & (df_asis["date"] >= x_min)].sort_values("date")
                if not df_asis_sku.empty:
                    fig.add_trace(go.Scatter(
                        x=df_asis_sku["date"], y=df_asis_sku["stock_on_hand_int"],
                        mode="lines", name="AS IS Stock",
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="lines", name="Total TO BE Stock",
            line=dict(color=PETROL, width=2),
            fill="tozeroy", fillcolor="rgba(46,110,126,0.10)",
        ))
        if df_asis is not None:
            df_asis_cls = df_asis[(df_asis["sku"].isin(skus_in_class)) & (df_asis["date"] >= x_min)].copy()
            df_asis_cls = df_asis_cls.groupby("date")["stock_on_hand_int"].sum().reset_index().sort_values("date")
            if not df_asis_cls.empty:
                fig.add_trace(go.Scatter(
                    x=df_asis_cls["date"], y=df_asis_cls["stock_on_hand_int"],
                    mode="lines", name="Total AS IS Stock",
                    line=dict(color="#C0392B", width=1.5),
                )) 
    fig.update_layout(
...
# ── Performance Metrics
metrics = compute_metrics(df_filtered, policy=policy_sel)
...
_is_minmax = policy_sel.lower().replace("-","_") == "min_max"
...
if _is_minmax:
    defs.append(("ss_medio",   "Avg. Safety Stock (S_min)", ",.2f", " units", ""))
    defs.append(("ss_medio_S", "Avg. Safety Stock (S_max)", ",.2f", " units", ""))
else:
    defs.append(("ss_medio", "Avg Safety Stock", ",.2f", " units", ""))

defs += [
    ("cobertura",   "Average Coverage",      ",.0f", " days",  ""),
    ("num_orders",  "Number of Orders",      ",.0f", " orders",""),
]
...
with st.spinner("Evaluating all combinations..."):
    _pm_frozen3  = tuple((m["filepath"], m["policy"], m["ns"], m["k"]) for m in parquets_meta)
    _abc_frozen3 = tuple(df_abc[["sku","Classe_ABC_XYZ"]].itertuples(index=False, name=None)) if df_abc is not None else ()
    res = find_best(_pm_frozen3, _abc_frozen3, bc_mode, sku_or_class, fill_rate_target)
