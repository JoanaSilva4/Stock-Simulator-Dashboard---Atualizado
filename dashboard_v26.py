...
if mode == "Overview":
    ...
    if df_abc is not None:
        ...
        for _cls_ov in sorted(df_abc["Classe_ABC_XYZ"].dropna().unique()):
            ...
            _res_ov  = find_best(_pm_frozen_ov, _abc_frozen_ov, "CLASS", _cls_ov, fill_rate_target=95.0)
            if _res_ov:
                ...
                if _meta_ov:
                    ...
                    _overview_comparison_rows.append({
                        ...
                        "TO BE Cost":          _m_ov.get("cost_total", np.nan),
                    })
    ...
    if df_abc is not None and "sku" in df_all.columns:
        ...
        for cls in classes_list:
            ...
            res_cls = find_best(_pm_frozen, _abc_frozen, "CLASS", cls, fill_rate_target=95.0)
            if res_cls:
    ...
    if df_abc is not None:
        ...
        for cls in classes_list_asis:
            ...
            res_cls = find_best(_pm_frozen2, _abc_frozen2, "CLASS", cls, fill_rate_target=95.0)
            if res_cls:
                best_cls, _, _ = res_cls
                best_meta = next(
                    (m for m in parquets_meta
                ...
                if best_meta:
                    ...
                    tobe_csl   = m_tobe.get("csl", np.nan)
                    tobe_ss    = m_tobe.get("ss_medio", np.nan)
                    tobe_ct    = m_tobe.get("cost_total", np.nan)
                    best_combo_lbl = f"{policy_label(best_cls['policy'])} · NS {best_cls['ns']}% · k={best_cls['k']}"
                else:
                    tobe_stock = tobe_sl = tobe_cov = tobe_csl = tobe_ss = tobe_ct = np.nan
                    best_combo_lbl = "—"
            else:
                tobe_stock = tobe_sl = tobe_cov = tobe_csl = tobe_ss = tobe_ct = np.nan
                best_combo_lbl = "—"
