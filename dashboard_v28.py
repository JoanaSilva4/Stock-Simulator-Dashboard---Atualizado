...
@st.cache_data(ttl=3600)
def _get_all_skus(filepaths_frozen):
...
with st.sidebar:
    ...
    if os.path.exists(_asis_path):
        try:
            df_asis = pd.read_parquet(_asis_path)[["sku", "date", "stock_on_hand_int"]]
    ...
    if mode == "By SKU":
        ...
        all_skus = _get_all_skus(tuple(m["filepath"] for m in parquets_meta))
        if not all_skus:
...
if mode == "Overview":
    ...
    st.markdown("<div class='sec-hdr'>◆ Simulated Performance (avg of the best combinations for the classes))</div>",
                unsafe_allow_html=True)
